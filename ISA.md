...
- [ ] ISC-43: Guarded KB write via `McpAgent` DO with elicitation confirm (fallback `confirm:true`) — declining aborts with no DB change; accepting writes + inserts exactly one `mcp_activity` row. [Slice 1 — DEFERRED: DO binding touches the deploy-button wrangler.jsonc + elicitation needs an MCP client to verify; deps installed, docs captured.]
- [ ] ISC-44: Built-in Assistant pane answers a read question on `env.AI`; opt-in Anthropic key via AI Gateway upgrades the tool-driving model. [Slice 2 — DEFERRED.]
...
- [ ] ISC-46: Anti: no write tool executes without auth → server-side Zod validation → audit row. [Slice 1 — DEFERRED with ISC-43; no write tool exists yet, so vacuously held — closes when the write path lands.]
